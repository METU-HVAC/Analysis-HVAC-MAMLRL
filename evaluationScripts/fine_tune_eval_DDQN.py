...
def fine_tune_policy(model, eval_env, run, energy_weight, env_type, model_env_type, steps=20, batch_size = 64, lr=1e-3):
    """
    ...
    for _ in range(steps):
        ...
        next_observations, _ = eval_env.reset()
        ...
        for step in range(0, max_ep_len - 1):
            ...
            if 'timestep' in info and len(info['timestep']) > 0 and step == max_ep_len - 1:
                    ...
                    next_observations,_ = eval_env.reset(seed=42)
                    done=True
    ...
    model.save_ddqn_agent(f"ddqn_fine_tuned_agent_weight_{energy_weight}_train_env_{model_env_type}_eval_env_{env_type}_episodes_{steps}.pth")
...
def main(args):
    ...
    experiment_name = f'DDQN-' + str(args.energy_weight) + '_' + climate + '_' + str(args.fine_tune) + '_' + args.model_env_type
    ...
    model.load_ddqn_agent(f"ddqn_agent_state_{args.energy_weight}_{args.model_env_type}.pth", device="cuda")

    
    if not args.use_fine_tuned_model and not f"ddqn_fine_tuned_agent_weight_{args.energy_weight}_train_env_{args.model_env_type}_eval_env_{args.env_type}.pth" in glob.glob("~/git/sinergym"):
        if args.fine_tune:
            model.train()
            fine_tune_policy(model, eval_env_fine_tune,run ,energy_weight=args.energy_weight, env_type=climate, model_env_type=args.model_env_type)
    else:
        # Load meta-trained parameters
        model.load_ddqn_agent(f"ddqn_fine_tuned_agent_weight_{args.energy_weight}_train_env_{args.model_env_type}_eval_env_{args.env_type}.pth", device="cuda")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model-env-type", type=str, default='mixed',
                        help="calls the model of a certain climate")
