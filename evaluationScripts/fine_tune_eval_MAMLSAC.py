...
def evaluate_sac(eval_env, model, run):
    ...
    for _ in range(1):
        ...
        for step in range(0, max_ep_len - 1):
            ...
            with torch.no_grad():
                logits = model.module.actor(state_tensor)
        ...
        # # Log the episodic return
        # run.log({"Evaluation Reward": np.mean(rewards_list) / (global_step + 1)})
        # run.log({"Evaluation Power Consumption":np.mean(power_demand_list)/ (global_step + 1)})
        # run.log({"Evaluation Temperature Violation":np.mean(temp_violation_list)/ (global_step + 1)})

def train_sac(envs, model, opt, device, meta_actor_loss_global, meta_critic_loss_global, energy_weight, env_type, model_env_type, steps=20):
    global_step = 0
    learner = model.clone()
    state, _ = envs.reset()
    model.module.train()
    ...
    for _ in range(0,steps):
        for step in range(0, max_ep_len - 1):
            ...
            state_tensor = torch.FloatTensor(state).to(device)
            with torch.no_grad():
                logits = learner.module.actor(state_tensor)
                action = torch.argmax(logits, dim=1)  # actions shape: [num_envs]
                # probs = F.softmax(logits, dim=-1)
                # m = Categorical(probs)
                # action = m.sample()
            ...
            learner.module.replay_buffer.store(state, action.cpu().numpy(), reward, next_state, done)
            if 'total_power_demand' in info and 'total_temperature_violation' in info:
                total_power_demand = info["total_power_demand"]
                total_temperature_violation = info["total_temperature_violation"]
                temp_violation_list += total_temperature_violation
            ...
            # Update model
            if step % 2048 == 0 and learner.module.replay_buffer.size >= 64:
    ...
    model.save_model(f"maml_sac_fine_tuned_agent_weight_{energy_weight}_train_env_{model_env_type}_eval_env_{env_type}_episodes_{steps}.pth")
    torch.save({"meta_optimizer_state_dict": opt.state_dict()}, f"maml_sac_fine_tuned_optimizer_weight_{energy_weight}_train_env_{model_env_type}_eval_env_{env_type}.pth")
...
def main(args):
    ...
    experiment_name = f'MAML-SAC-' + str(args.energy_weight) + '_' + climate + '_' + str(args.fine_tune) + '_' + args.model_env_type
    ...
    model.load_model(f"MAML_sac_agent_checkpoint_{args.energy_weight}_{args.model_env_type}.pth")
    ...
    if not args.use_fine_tuned_model and not (f"maml_sac_fine_tuned_agent_weight_{args.energy_weight}_train_env_{args.model_env_type}_eval_env_{args.env_type}.pth" in glob.glob("~/git/sinergym")
                                           and f"maml_sac_fine_tuned_optimizer_weight_{args.energy_weight}_train_env_{args.model_env_type}_eval_env_{args.env_type}.pth" in glob.glob("~/git/sinergym")):  
        if args.fine_tune:
            train_sac(eval_env, maml, opt, device, meta_actor_loss_global, meta_critic_loss_global, energy_weight=args.energy_weight, env_type=args.env_type, model_env_type=args.model_env_type)    
    else:
        # Load meta-trained parameters
        model.load_ddqn_agent(f"maml_sac_fine_tuned_agent_weight_{args.energy_weight}_train_env_{args.model_env_type}_eval_env_{args.env_type}.pth", device="cuda")
        opt.load_state_dict(torch.load(f"maml_sac_fine_tuned_optimizer_weight_{args.energy_weight}_train_env_{args.model_env_type}_eval_env_{args.env_type}.pth", map_location=device)["meta_optimizer_state_dict"])
    
    # Evaluate the fine-tuned policy
    model.eval()
    evaluate_sac(eval_env, maml, run)
